...
function setSpecificTime(date, hours, minutes, seconds, milliseconds) {
  ...
  dateNew.setMilliseconds(milliseconds);
  return dateNew;
}
...
function nextNewYear() {
  ...
  let minutesToNewYear = (diff / (1000 * 60)) % 60;
  let secondsToNewYear = (diff / 1000) % 60;
  let millisecondsToNewYeat = diff % 1000;
  return {
    days: daysToNewYear,
    hours: hoursToNewYear,
    minutes: minutesToNewYear,
    seconds: secondsToNewYear,
    milliseconds: millisecondsToNewYeat,
  };
}
...
function getDayOfWeek(date) {
  // Перевірка, чи є вхідне значення об'єктом Date,це можно зробити перевіривши чи є  по типу функdate.getTimeція .
  // Якщо date не є об'єктом Date, повертаємо рядок
  // "Помилка: вхідне значення має бути об'єктом Date"
  // Збереження початкової дати для виведення  в форматі ISO.
...
function getDaysInMonth(date) {
  ...
  if (typeof date.getTime !== "function")
    return "Помилка: вхідне значення має бути об'єктом Date";
  const inputDate = new Date(date).toISOString();
  const data2 = new Date(date.getFullYear(), date.getUTCMonth() + 1, 1);
  const oneDay = 1000 * 60 * 60 * 24;
  const date3 = new Date(data2 - oneDay);
  return { inputDate: inputDate, getDaysInMonth: date3.getDate() };
}
